...
def make_confusion_matrix(ground_truths, predicted_classes, ground_truths_test, predicted_classes_test, n_classes = 2):
    ...
    plt.savefig('Warsztaty-badawcze/confusion_matrix_train.png')  # Saving the confusion matrix for train data as an image
    ...
    plt.savefig('Warsztaty-badawcze/confusion_matrix_test.png')  # Saving the confusion matrix for test data as an image
    plt.close()  # Closing the plot to release memory
...
def perform_epoch(mil_model, dataloader, optimizer, loss_function):
    ...
    return np.mean(losses), metrics.balanced_accuracy_score(ground_truths, predicted_classes), ground_truths, predicted_classes
...
def main(hyper_parameters):
    ...
    loss_test, bac_test, ground_truths_test, predicted_classes_test = testing(sparseconvmil_model, dataloader_test, loss_function)
    print("Test results: ", f' loss = {loss_test:.3f}', f'bac = {bac_test:.3f}')
    print("Ground truths test: ", ground_truths_test)
    print("Predicted classes test: ", predicted_classes_test)

    epoch_df = pd.DataFrame(epoch_data, columns=['Epoch', 'Loss', 'BAC'])

    #csv_path = 'Warsztaty-badawcze/train_epochs.csv'
