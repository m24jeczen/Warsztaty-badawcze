...
class SparseConvMIL(nn.Module):
    def __init__(self, tile_embedder: nn.Module, sparse_cnn: nn.Module, wsi_embedding_classifier: nn.Module,
                 ...
                 tile_coordinates_flips_augmentation: bool, tile_coordinates_resize_augmentation: bool, n_output_channels_tile_embedding):
        super().__init__()
        ...
        n_output_channels_tile_embedding = None
    ...
    def printModel(self):
        print(f"tile embedder, {self.tile_embedder}")
        print(f"sparse_cnn, {self.sparse_cnn}")
        print(f"wsi_embedding_classifier, {self.wsi_embedding_classifier}")
        print(f"sparse_map_downsample, {self.sparse_map_downsample}")
        print(f"tile_coordinantes_rotation_augumentation, {self.tile_coordinates_rotation_augmentation}")
        print(f"tile_coordinates_flips_augumentation, {self.tile_coordinates_flips_augmentation}")
        print(f"tile_coordinates_resize_augumentation, {self.tile_coordinates_resize_augmentation}")
    ...
    def forward(self, x, tiles_original_locations, training=True):
...
def instantiate_sparseconvmil(tile_embedder_resnet_architecture, tile_embedder_pretrained, n_out_channels_conv1,
    ...
    print("n_output_channels_tile_embedding: ", n_output_channels_tile_embedding)
    ...
    sparseconvmil_model = SparseConvMIL(tile_embedder, sparse_input_pooling, wsi_embedding_classifier,
                                        sparse_map_downsample, True, True, True, n_output_channels_tile_embedding)
