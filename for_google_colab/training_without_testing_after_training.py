...
def make_confusion_matrix(training, ground_truths, predicted_classes, n_classes = 2):
    # Confusion matrix for train data
    if (training):
      conf_matrix = confusion_matrix(ground_truths, predicted_classes)
    ...
    if (training):
      ...
      plt.savefig('Warsztaty-badawcze/conf_matrixes/confusion_matrix_random_train_ZUPA.png')  # Saving the confusion matrix for train data as an image
    ...
    if not (training):
        ...
        plt.savefig('Warsztaty-badawcze/conf_matrixes/confusion_matrix_random_testB_ZUPA.png')  # Saving the confusion matrix for test data as an image
        plt.close()  # Closing the plot to release memory
...
def main(hyper_parameters):
    ...
    if hyper_parameters['load_model_path'] and os.path.exists(hyper_parameters['load_model_path']):
        start_epoch, sparseconvmil_model = load_model(sparseconvmil_model, optimizer, hyper_parameters['load_model_path'])
    ...
    if not (training):
        sparseconvmil_model.eval()
        print('Checking on test data ')
        loss_test, bac_test, recall_test, ground_truths, predicted_classes = testing(sparseconvmil_model, dataloader_test, loss_function)
        print("Test results:", f'    loss={loss_test:.3f}', f'    bac={bac_test:.3f}', f'    recall={recall_test:.3f}')
        print("Ground truths test: ", ground_truths)
        print("Predicted classes test: ", predicted_classes)
    ...
    make_confusion_matrix(training, ground_truths, predicted_classes)
