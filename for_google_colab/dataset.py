...
class Dataset(torch.utils.data.dataset.Dataset):
    ...
    def __init__(self, slides_parent_folder, slides_label_filepath, n_sampled_tiles_per_slide,
        ...
        valid_slide_ids = [slide_id for slide_id in self.slides_ids if slide_id != '.ipynb_checkpoints']
        assert all([slide_id in self.slide_labels.keys() for slide_id in valid_slide_ids])
    ...
    def load_tile_paths(self) -> (list, list):
        ...
        slide_folders_ids = [os.path.basename(f) for f in slide_folders if os.path.basename(f) != '.ipynb_checkpoints']

        # Get absolute tiles files
        # Get absolute tiles files, excluding .ipynb_checkpoints folder
        tiles_paths = []
        for slide_folder in slide_folders:
            if os.path.basename(slide_folder) != '.ipynb_checkpoints':
                tiles_paths.append(
        ...
        tiles_locations = list(map(lambda tiles_files_per_slide:
                                   ...
                                   tiles_paths))
        print("Length of slide_folders_ids:", len(slide_folders_ids))
        print("Length of tiles_locations:", len(tiles_locations))
        print("Length of tiles_paths:", len(tiles_paths))

        print("slide_folders_ids:", slide_folders_ids)
        print("tiles_locations:", tiles_locations)
